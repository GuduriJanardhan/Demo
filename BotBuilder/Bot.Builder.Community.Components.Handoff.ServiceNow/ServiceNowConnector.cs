...
namespace Bot.Builder.Community.Components.Handoff.ServiceNow
{
    ...
    {
        public static async Task<ServiceNowConversationRecord> EscalateToAgentAsync(ITurnContext turnContext, IEventActivity handoffEvent, string serviceNowTenant, string serviceNowUserName, string serviceNowPassword, ConversationHandoffRecordMap conversationHandoffRecordMap)
        {
            ...
            return new ServiceNowConversationRecord { ConversationId = turnContext.Activity.Conversation.Id, ServiceNowTenant = serviceNowTenant, ServiceNowUsername = serviceNowUserName, ServiceNowPassword = serviceNowPassword, Timezone = timeZone, UserId = userId, EmailId = emailId };
        }
